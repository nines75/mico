...
直後に書いた括弧内の文字列をルールとして解釈します。  
コメント記号だけはこのディレクティブでのエスケープに対応していないため、エスケープ記号(`\`)を直前に使う必要があります。

これはディレクティブが利用可能なフィルターでのみ使用できます。

## 例

```
!rule # 'rule'(strictルール)と解釈される
@escape(!rule) # '!rule'と解釈される
```
...
strictルールはかんたんコメントの非表示を除いた全てのルールに先行して適用されます。
...
ただし、ポップアップから無効化したコマンドの数が確認できます。

無効化ルールは非表示ルール(通常のルール)の後に適用されます。
...
@disable
red # 'red'というコマンドを無効化
big # 非表示ルールと競合しているため無効
all # すべてのコマンドを無効化
@end

big # 'big'というコマンドを含むコメントを非表示
...
基本的にルールはフィルターの先頭から順番に適用されますが、一部のルールは別のルールに先行して適用されることがあります。  
